CLASS lhc_zrap100_r_traveltp_kar DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    CONSTANTS:
...
CLASS lhc_zrap100_r_traveltp_kar IMPLEMENTATION.
  METHOD get_global_authorizations.
  ENDMETHOD.
  METHOD earlynumbering_create.

    DATA:
      entity           TYPE STRUCTURE FOR CREATE zrap100_r_traveltp_kar,
    ...
    ELSE.
      ...
      SELECT SINGLE FROM zrap100_atravkar FIELDS MAX( travel_id ) AS travelID INTO @travel_id_max.
      "Get max travel ID from draft table
      SELECT SINGLE FROM zrap100_dtravkar FIELDS MAX( travelid ) INTO @DATA(max_travelid_draft).
      IF max_travelid_draft > travel_id_max.
        travel_id_max = max_travelid_draft.
      ENDIF.
      "Get max travel ID from active table
      SELECT SINGLE FROM /dmo/travel FIELDS MAX( travel_id ) INTO @DATA(max_travel_id).
      IF max_travel_id > travel_id_max.
        travel_id_max = max_travel_id.
      ENDIF.
    ENDIF.
  ...
  METHOD setStatusToOpen.
    ...
    READ ENTITIES OF zrap100_r_traveltp_kar IN LOCAL MODE
     ENTITY Travel
    ...
    MODIFY ENTITIES OF ZRAP100_R_TravelTP_kar IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE ZRAP100_R_TravelTP_KAR.
  ...
  METHOD copyTravel.
    DATA:
       travels       TYPE TABLE FOR CREATE zrap100_r_traveltp_kar\\travel.

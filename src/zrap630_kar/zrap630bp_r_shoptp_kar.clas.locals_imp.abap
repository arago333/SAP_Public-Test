...
CLASS LHC_SHOP IMPLEMENTATION.
  ...
  READ ENTITIES OF ZRAP630R_ShopTP_KAR IN LOCAL MODE
    ENTITY Shop
  ...
  SELECT MAX( ORDER_ID ) FROM ZRAP630_ASHOPKAR INTO @DATA(max_object_id).
  "Add support for draft if used in modify
  "SELECT SINGLE FROM FROM ZRAP630SH00D_KAR FIELDS MAX( OrderID ) INTO @DATA(max_orderid_draft). "draft table
  "if max_orderid_draft > max_object_id
  " max_object_id = max_orderid_draft.
  "ENDIF.
  MODIFY ENTITIES OF ZRAP630R_ShopTP_KAR IN LOCAL MODE
    ENTITY Shop

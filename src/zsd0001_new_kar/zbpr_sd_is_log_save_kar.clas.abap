  ...
  METHOD fetch_and_save.
    ...
    TRY.
        ...
        lo_req1->set_uri_path(
          i_uri_path = |{ lv_base_url }&$top={ iv_top }&$skip={ iv_skip }| ).
    ...
    LOOP AT lt_save INTO DATA(ls_save).
      ...
      IF sy-subrc = 0.
        " 기존 건 → DB 그대로 유지, MODIFY 대상 제외
        CONTINUE.
      ELSE.
        IF ls_save-statusis = 'X'.
          " FAILED → StatusIN 공백, 006 API 없이 바로 저장
          ls_save-statusin = ' '.
